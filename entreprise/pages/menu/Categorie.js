...
const Categorie = (props/*: PropsCat*/) => {
  ...
  const Activate=()=>{
    ...
    sortie.push(<View>
      <TouchableOpacity
            onPress={()=>{axios.post("http://localhost:3000/menu/activate",{id:produit.id,ctif:produit.actif})}}><Text>{produit.actif==0?"activer":"desactiver"} la categorie {produit.name}</Text>
            </TouchableOpacity>
            <TouchableOpacity
            onPress={()=>{axios.post("http://localhost:3000/menu/delete",{id:produit.id})}}><Text>supprimer le produit {produit.name}</Text>
            </TouchableOpacity>
              </View>)})
              return <>{
